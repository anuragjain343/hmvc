...
class Stripe{
    ...
    public function __construct () {
        ...
        $publishable_key = STRIPE_PUBLISABLE_KEY;
    ...
    function create_plan($product_id,$interval_count, $currency,  $amount, $nickname=''){
        try{
    ...
    function create_subscription($customer_id, $plan_id,$coupon_code){
    ...
    function cancel_subscription($subscription_id, $at_period_end = false){
      ...
      function cancel_mysubscription($subscription_id, $at_period_end = true){
        try{
        $sub = \Stripe\Subscription::retrieve($subscription_id);
        //$subscription = $sub->cancel(['at_period_end' => $at_period_end]);
       $upda =  \Stripe\Subscription::update($subscription_id,['cancel_at_period_end' => true,]
         );
            //pr($sub);
            ...
            return array('status'=>true,'message'=>'Retrieved successfully', 'data'=>$upda); //success
    ...
    //code for update plans 

    function update_plan($plan_id,$attrs){
           //$upArr = array();

       
   
        try{
            //$plan = \Stripe\Plan::update($plan_id,['nickname' => 'new level']);
            $plan = \Stripe\Plan::update($plan_id,array('nickname'=>$attrs));
            if(empty($plan)){
                return array('status'=>false,'message'=>'Something went wrong');
            }
            
            return array('status'=>true,'message'=>'Plan updated successfully', 'data'=>$plan); //success
    ...
    /*code for create coupon update_coupons*/
   function create_coupon($name, $percent_off, $duration){
        try{
            ...
            return array('status'=>true,'message'=>'Coupon created successfully', 'data'=>$coupon); //success
    ...
    function update_coupon($couponId,$name){

        try{
            //$plan = \Stripe\Plan::update($plan_id,['nickname' => 'new level']);
            $coupon = \Stripe\Coupon::update($couponId,array('name'=>$name));
            if(empty($coupon)){
                return array('status'=>false,'message'=>'Something went wrong');
            }
            
            return array('status'=>true,'message'=>'Coupon updated successfully', 'data'=>$coupon); //success
