...
function get_membership_price(){
  ...
  return $pricr = array('level1'=>7.99,'level2'=>39.99,'level3'=>89.99,'level4'=>150);
}

function  get_membership_price_discount($price){

  return $total= (150-(150*$price)/100);
}

if ( ! function_exists('get_notification_url')) {
    function get_notification_url($data,$type,$userId=''){
      switch ($data) {

        case 'article':
            $url = base_url().'offers';
            if($type =='delete_article'){
              $url = base_url().'offers#myOffer';
            }
            break;
        ...
        case 'teases_updates':
            ...
            if($type =='other'){
              ...
              $url = base_url().'user/userDetail/'.$id;
            }
            break;
        case 'view_updates':
            $url = base_url().'view';
...
/***********  Any new project specific helper method can be added below Monthly ***********/

/*code by sunil*/
function get_membership_month(){
  return $membership_month = array('1'=>'Monthly','3'=>'Every 3 months','6'=>'Every 6 months','12'=>'Yearly');
}
function get_plan_level(){
  return $plan_level = array('1'=>'Level 1','2'=>'Level 2','3'=>'Level 3','4'=>'Level 4');
}
function get_coupon_duration(){
  return $coupon = array('forever'=>'Forever','once'=>'Once');
}
...
//end of 2
  function verify_user_auth_cookie($cookie_name){
   $CI =& get_instance();
    ...
    $remData = $CI->common_model->GetSingleJoinRecord('auth_token',$field_first,USERS,$field_second,$field_val,$where); 
    if(empty($remData)){
      return FALSE;
    }
    if(!password_verify($validator,$remData->hashedValidator)){
        return FALSE;
    }
    //token matched! get corresponding user detail and store it in session
    $user_data_arr['userId']   = $remData->id;
    $user_data_arr['emailId']  = $remData->email;
    $user_data_arr['name']     = $remData->fullName;
    $user_data_arr['image']    = $remData->profileImage;
    $user_data_arr['UserRole'] = $remData->userRole;
    $user_data_arr['userPlan'] = $remData->userPlan;
    $user_data_arr['isLogin']  = TRUE;
    $_SESSION[USER_SESS_KEY]  = $user_data_arr;
