...
class Common_Front_Controller extends MX_Controller {
    ...
    public function check_user_session(){
       ...
       // pr($page_slug);
        $allowed_pages = array('index','home'); //these pages/methods do not require user authentication
    ...
    function logout($is_redirect=TRUE){
       ...
       // pr($_SESSION[$this->user_session_key]);
        
         if($_COOKIE['mvt_remember_me_token']){
           $selector = $_COOKIE['mvt_remember_me_token'];
            $arra = explode(':',$selector);
            $where = array('selector'=>$arra[0]);
             $CI = & get_instance(); 
            $CI->load->model('common_model');
            $remove = $CI->common_model->deleteData('auth_token',$where);
            setcookie("mvt_remember_me_token", "", time() - 3600, '/');
         }
        /*  if(!empty($_COOKIE['reffralId'])){
           setcookie('reffralId', "", time() - 3600);
         }*/
    ...
    public function check_ajax_auth(){
        ...
        $where = array('id'=>$user_data['userId'],'status'=>1); //status:1 means active 
        $check = $this->common_model->is_data_exists($this->tbl_users,$where);
