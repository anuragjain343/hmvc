...
<div class="app-content content" >
 <div class="content-wrapper">        
  <div class="content-wrapper-before Hedtxt">
      <h2 class="ml-2">Users (<?php if(!empty($total_rows)){ echo $total_rows;  }else{ echo '0'; } ?>)</h2>
  </div>
...
function show_loader(){
 ...
 $('#tl_admin_loader').hide();
  ...
  //
    var data = $("#hash");
    ajax_fun('users/user_List');
    function ajax_fun(url)

    {  
        var csrf_test_name=data.attr('data-name');
        var value=data.attr('data-value');
        var dataObject={page:url,csrf_test_name:value,search:search_text,id:customSelect};
    $.ajax({
        type:'POST',
        url:url,
        data:dataObject,
